...
var uniPopup = function uniPopup() {return __webpack_require__.e(/*! import() | components/uni-popup/uni-popup */ "components/uni-popup/uni-popup").then(__webpack_require__.bind(null, /*! @/components/uni-popup/uni-popup.vue */ "E:\\project\\myApplication\\components\\uni-popup\\uni-popup.vue"));};var tkiFileManager = function tkiFileManager() {return __webpack_require__.e(/*! import() | components/tki-file-manager */ "components/tki-file-manager").then(__webpack_require__.bind(null, /*! @/components/tki-file-manager.vue */ "E:\\project\\myApplication\\components\\tki-file-manager.vue"));};var _default = { components: { tkiFileManager: tkiFileManager, uniPopup: uniPopup }, data: function data() {return { index: 0, array: [], //['新增', '续贷', '收回再贷']
      ...
      array7_1Info: [], isdis: false, businessData: { title: "", // 申报名称
      ...
      userIds: "", seq: "", ishold: false };}, onNavigationBarButtonTap: function onNavigationBarButtonTap(e) {this.isKeep();}, onLoad: function onLoad() {this.businessData.applay_date = (0, _util.formateDate)(new Date(), "Y-M-D");var that = this;uni.request({ url: "http://" + (0, _config.baseIp)() + "/ams/system/distribute.htm?module=pullDown&XLX=sbType", success: function success(res) {var data = res.data;that.arrayInfo = data.object;data = data.object.map(function (item) {return item.name;});that.array = data;that.businessData.title = that.arrayInfo[0].code;} });uni.request({ url: "http://" + (0, _config.baseIp)() + "/ams/system/distribute.htm?module=pullDown&XLX=customerType", success: function success(res) {var data = res.data;that.array2Info = data.object;data = data.object.map(function (item) {return item.name;});data.unshift("请选择客户类型");that.array2 = data;} });uni.request({ url: "http://" + (0, _config.baseIp)() + "/ams/system/distribute.htm?module=pullDown&XLX=level_modle", success: function success(res) {var data = res.data;that.array4Info = data.object;data = data.object.map(function (item) {return item.name;});data.unshift("请选择拟初评结果");that.array4 = data;} });uni.request({ url: "http://" + (0, _config.baseIp)() + "/ams/system/distribute.htm?module=pullDown&XLX=levelType", success: function success(res) {var data = res.data;that.array6Info = that.array5Info = data.object;data = data.object.map(function (item) {return item.name;});var data2 = [].concat(data);data.unshift("请选择拟初评结果");data2.unshift("请选择上季度拟初评结果");that.array5 = data;that.array6 = data2;} });uni.request({ url: "http://" + (0, _config.baseIp)() + "/ams/system/distribute.htm?module=check_dept", success: function success(res) {var data = res.data;that.array3Info = that.array7Info = data.object;var dataInfo = data.object.map(function (item) {return item.dept_name;});var data2Info = [].concat(dataInfo);dataInfo.unshift("请选择营销责任人");that.array3 = dataInfo;data2Info.unshift("请选择协助调查岗");that.array7 = data2Info;} });}, beforeDestroy: function beforeDestroy() {return false;}, methods: { bindPickerChange: function bindPickerChange(e) {this.index = e.target.value;this.businessData.title = this.arrayInfo[e.target.value].code;}, bindPickerChange2: function bindPickerChange2(e) {this.index2 = e.target.value;if (e.target.value == 0) {uni.showToast({ title: "选择失败", icon: "none" });return;}this.businessData.cust_type = this.array2Info[e.target.value - 1].code;}, bindPickerChange3: function bindPickerChange3(e) {var that = this;this.index3 = e.target.value;if (e.target.value == 0) {uni.showToast({ title: "选择失败", icon: "none" });return;}uni.request({ url: "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=check_person&dept_id=").concat(that.array3Info[e.target.value].id - 1), success: function success(res) {var data = res.data;if (data.object.length == 0) {uni.showToast({ title: "选择失败，请重新选择！", icon: "none" });that.index3 = 0;that.array3_1 = [];that.businessData.userIds2 = "";} else {that.array3_1Info = data.object;uni.showToast({ title: "请选择具体信息",
              icon: "none" });
    ...
    submitAcce: function submitAcce() {
      if (!this.ishold) {
        uni.showModal({
          ...
          success: function success(res) {
            if (res.confirm) {
              console.log('用户点击确定', " at pages\\application\\business\\business.vue:456");
            } else if (res.cancel) {
              console.log('用户点击取消', " at pages\\application\\business\\business.vue:458");
            }
          } });
        ...
        uni.request({
          url: "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=submitApp&id=").concat(that.seq, "&userIds=").concat(that.userIds),
          success: function success(res) {
            var data = res.data;
            uni.hideLoading();
            if (data.code == 1) {
          ...
          fail: function fail(err) {
            console.log(err, " at pages\\application\\business\\business.vue:481");
          } });
    ...
    retrieval: function retrieval() {
      ...
      if (this.businessData.customer_name == "") {
        ...
        uni.request({
          ...
          fail: function fail(err) {
            console.log(err, " at pages\\application\\business\\business.vue:539");
            uni.hideLoading();
          } });
        ...
        success: function success(res) {
          if (res.confirm) {
            ...
            uni.request({
              ...
              fail: function fail(err) {
                console.log(err, " at pages\\application\\business\\business.vue:656");
                uni.hideLoading();
              } });
