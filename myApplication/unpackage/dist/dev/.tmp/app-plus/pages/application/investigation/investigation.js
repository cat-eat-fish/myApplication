...
}var _default =
{
  ...
  onLoad: function onLoad(e) {
    ...
    var url = "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=information&user_id=").concat((0, _service.getUserInfo)().userId, "&acceptid=").concat(e.acceptid, "&pdid=").concat(e.pdid, "&tiid=").concat(e.tiid, "&piid=").concat(e.piid, "&docid=").concat(that.doc_id);
    // console.log(url)
    uni.request({
      url: url,
      success: function success(res) {
        var data = res.data;
        console.log(data, " at pages\\application\\investigation\\investigation.vue:553");
  ...
  methods: {
    ...
    doUpload: function doUpload() {
      ...
      console.log(url, " at pages\\application\\investigation\\investigation.vue:619");
      // return;
      uni.uploadFile({
        url: url,
        filePath: "",
    ...
    radioChange4: function radioChange4(evt) {
      for (var i = 0; i < this.items4.length; i++) {
        if (this.items4[i].value === evt.target.value) {
          this.current4 = i;
          console.log(this.nodeListInfo[i], " at pages\\application\\investigation\\investigation.vue:753");
    ...
    // 退回
    back: function back() {
      console.log(this.tokenId, " at pages\\application\\investigation\\investigation.vue:869");
      // 验证
      if (this.tokenId === "") {
      ...
      console.log(url, " at pages\\application\\investigation\\investigation.vue:878");
      uni.showLoading({ title: "退回中", mask: true });
      uni.request({
        url: url,
        success: function success(res) {
          var data = res.data;
          console.log(res, " at pages\\application\\investigation\\investigation.vue:884");
          if (data.code == 1) {
            uni.hideLoading();
    ...
    submitAcce: function submitAcce() {
      ...
      if (this.inve == "回复") {
        console.log(this.date, this.date2, " at pages\\application\\investigation\\investigation.vue:953");
      ...
      uni.request({
        ...
        fail: function fail(err) {
          console.log(err, " at pages\\application\\investigation\\investigation.vue:983");
        } });
