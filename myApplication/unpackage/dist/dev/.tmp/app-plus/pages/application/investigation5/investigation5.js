...
var uniPopup = function uniPopup() {return __webpack_require__.e(/*! import() | components/uni-popup/uni-popup */ "components/uni-popup/uni-popup").then(__webpack_require__.bind(null, /*! @/components/uni-popup/uni-popup.vue */ "E:\\project\\myApplication\\components\\uni-popup\\uni-popup.vue"));};function getDate(type) {var date = new Date();var year = date.getFullYear();var month = date.getMonth() + 1;var day = date.getDate();if (type === 'start') {year = year - 60;} else if (type === 'end') {year = year + 2;}month = month > 9 ? month : '0' + month;;day = day > 9 ? day : '0' + day;return "".concat(year, "-").concat(month, "-").concat(day);}var _default = { components: { uniPopup: uniPopup }, data: function data() {return { pageActive: false, // 当前任务 -- 审查
      ...
      tiid: "", pdId: "", piid: "", doc_id: "", status: 0, mianId: "", comment: "", dataInfo: { load_type: "" }, Enclosure: [], //附件
      type: "", // 退回
      backReasion: "", //退回理由
      tokenId: "", //退回节点id
      nodeId: "", nodecode: "", nodeListInfo: [], nodeList: [] // 退回
        ...
        that.array2Info = data.object;var dataInfo = data.object.map(function (item) {return item.dept_name;});dataInfo.unshift("请选择人员");that.array2 = dataInfo;} });var url = "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=informationYX_YD&user_id=").concat((0, _service.getUserInfo)().userId, "&acceptid=").concat(e.acceptid, "&pdid=").concat(e.pdid, "&tiid=").concat(e.tiid, "&piid=").concat(e.piid, "&docid=").concat(that.doc_id);console.log(url, " at pages\\application\\investigation5\\investigation5.vue:634");uni.request({ url: url, success: function success(res) {var data = res.data;console.log(data, " at pages\\application\\investigation5\\investigation5.vue:639");that.dataInfo = data.object[0]; // that.Enclosure = data.object1;
        // that.doc_id = data.object[0].documentid
        that.mianId = data.object[0].id;that.now = data.object[0].tokenName;uni.setNavigationBarTitle({ title: data.object[0].tokenName });that.inve = data.object[0].tokenName;if (data.object1.length != 0) {that.votiResult = data.object1.map(function (item) {return "".concat(item.USERNAME, " : ").concat(item.STATUS2);});that.votiStatus = true;var nowInfo = data.object1.filter(function (item) {return item.USERNAME == (0, _service.getUserInfo)().name;});that.isvoti = true;if (nowInfo[0] != undefined) {that.current = nowInfo[0].STATUS == 1 ? 0 : 1;that.status = nowInfo[0].STATUS;that.thistp = true;} else {that.thistp = false;}} else {that.votiStatus = false;that.thistp = false;}} });this.getNodeList();this.getpj();}, methods: { // 选择时间
    ...
    getNodeList: function getNodeList() {var that = this,url = "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=backNodeList&piId=").concat(that.piid);uni.request({ url: url, success: function success(res) {var data = res.data; // console.log(data);
          that.nodeListInfo = data.object;data.object.map(function (item) {item.value = item.NAME_;item.name = item.NAME_;});that.items4 = data.object;} });}, radioChange: function radioChange(evt) {for (var i = 0; i < this.items.length; i++) {if (this.items[i].value === evt.target.value) {this.current = i;this.status = evt.target.value == "tongyi" ? 1 : 0;break;}}}, radioChange4: function radioChange4(evt) {for (var i = 0; i < this.items4.length; i++) {if (this.items4[i].value === evt.target.value) {this.current4 = i;console.log(this.nodeListInfo[i], " at pages\\application\\investigation5\\investigation5.vue:761");this.tokenId = this.nodeListInfo[i].TOKENID;this.nodeId = this.nodeListInfo[i].NODEID;this.tiid = this.nodeListInfo[i].TIID;break;}}}, radioChange5: function radioChange5(evt) {for (var i = 0; i < this.items5.length; i++) {if (this.items5[i].value === evt.target.value) {this.current5 = i;this.votiStatus = evt.target.value == "tongyi" ? 1 : 0;break;}}}, bindPickerChange2: function bindPickerChange2(e) {this.index2 = e.target.value;if (e.target.value == 0) {uni.showToast({ title: "请重新选择", icon: "none" });return;}this.inveder = String(this.array2Info[e.target.value - 1].id);console.log(this.inveder, " at pages\\application\\investigation5\\investigation5.vue:785");}, radioChange2: function radioChange2(evt) {for (var i = 0; i < this.items2.length; i++) {if (this.items2[i].value === evt.target.value) {this.current2 = i;break;}}}, radioChange3: function radioChange3(evt) {for (var i = 0; i < this.items3.length; i++) {if (this.items3[i].value === evt.target.value) {this.current3 = i;break;}}}, bindPickerChange7: function bindPickerChange7(e) {var that = this;this.index7 = e.target.value;if (e.target.value == 0) {uni.showToast({ title: "选择失败", icon: "none" });return;}uni.request({ url: "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=check_person&dept_id=").concat(that.array7Info[e.target.value].id - 1), success: function success(res) {var data = res.data;if (data.object.length == 0) {uni.showToast({ title: "选择失败，请重新选择！" });that.array7_1 = [];that.businessData.userIds = "";} else {that.array7_1Info = data.object;uni.showToast({ title: "请选择具体信息", icon: "none" });var dataInfo = data.object.map(function (item) {return item.dept_name;});that.array7_1 = dataInfo;that.tprList.push({ name: that.array7_1Info[0].dept_name, code: that.array7_1Info[0].id });}} });}, bindPickerChange7_1: function bindPickerChange7_1(e) {this.index7_1 = e.target.value;this.tprList.push({ name: this.array7_1Info[e.target.value].dept_name, code: this.array7_1Info[e.target.value].id });}, // 审议 投票
    doVote: function doVote() {if (this.votiStatus == false) {uni.showToast({ title: "请投票，并填写投票意见！", icon: "none" });return;}uni.showLoading({ title: "投票中", mask: true });var that = this,url = "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=saveVote2_YD&userId=").concat((0, _service.getUserInfo)().userId, "&status=").concat(that.votiStatus, "&comment=").concat(that.votiOpinion, "&taskInstanceId=").concat(that.tiid, "&acceptId=").concat(that.acceptid, "&docId=").concat(that.doc_id, "&tokenName=").concat(that.inve, "&pdid=").concat(that.pdid, "&nodeCode=").concat(that.nodecode);console.log(url, " at pages\\application\\investigation5\\investigation5.vue:853");uni.request({ url: url, success: function success(res) {var data = res.data;if (data.code == 1) {that.isvoti = true;that.pageActive = !that.pageActive;uni.hideLoading();uni.showToast({ title: data.message, duration: 3000, mask: true });setTimeout(function () {uni.reLaunch({ url: "/pages/tabBar/information/information" });}, 3000);}} });}, // 审议
    // 删除审议投票人
    delList: function delList(e) {this.tprList.splice(e.target.dataset.value, 1);}, // 附件下载
    downEnclosure: function downEnclosure() {if (this.Enclosure.length == 0) {uni.showToast({ title: "暂无附件", icon: "none" });} else {console.log("有附件", " at pages\\application\\investigation5\\investigation5.vue:882");}}, // 退回
    back: function back() {console.log(this.tokenId, " at pages\\application\\investigation5\\investigation5.vue:887"); // 验证
      if (this.tokenId === "") {uni.showToast({ title: "请选择退回节点", icon: "none" });return;}var that = this;var url = "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=goBack&userId=").concat((0, _service.getUserInfo)().userId, "&pdId=").concat(that.pdid, "&tiId=").concat(that.tiid, "&nodeId=").concat(that.nodeId, "&piId=").concat(that.piid, "&tokenId=").concat(that.tokenId, "&docId=").concat(that.doc_id, "&backReasion=").concat(that.backReasion, "&tokenName2=").concat(that.inve);console.log(url, " at pages\\application\\investigation5\\investigation5.vue:896");uni.showLoading({ title: "退回中", mask: true });uni.request({ url: url, success: function success(res) {var data = res.data;console.log(res, " at pages\\application\\investigation5\\investigation5.vue:902");if (data.code == 1) {uni.hideLoading();uni.showToast({ title: data.message, mask: true, duration: 3000 });setTimeout(function () {uni.reLaunch({ url: "/pages/tabBar/information/information?isRefresh=true" });}, 3000);}} });}, // 提交审核
    submitAcce: function submitAcce() {var that = this;var url = "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=approveYX_YD&userId=").concat((0, _service.getUserInfo)().userId, "&taskInstanceId=").concat(that.tiid, "&mainId=").concat(that.mianId, "&pdId=").concat(that.pdid, "&piId=").concat(that.piid, "&docId=").concat(that.doc_id, "&comment=").concat(that.comment, "&status=").concat(that.status, "&userIds=").concat(that.inveder, "&acceptId=").concat(that.acceptid);if (this.inve == "分网点审查（一般新增）") {if (this.tprList.length < 2 || this.tprList.length > 3) {uni.showToast({ title: "请选择两个或三个投票人", icon: "none" });return;}this.tprList = (0, _util.deteleObject)(this.tprList);var a = [];this.tprList.map(function (item) {a.push(item.code);});this.inveder = a.join(',');var that = this;var url = "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=approveYX_YD&userId=").concat((0, _service.getUserInfo)().userId, "&taskInstanceId=").concat(that.tiid, "&mainId=").concat(that.mianId, "&pdId=").concat(that.pdid, "&piId=").concat(that.piid, "&docId=").concat(that.doc_id, "&comment=").concat(that.comment, "&status=").concat(that.status, "&userIds=").concat(that.inveder, "&acceptId=").concat(that.acceptid);}if (this.inve == "分网点审议（一般新增）") {if (!this.isvoti) {uni.showToast({ title: "请先投票！", icon: "none" });return;}if (this.dataInfo.yj_num != this.dataInfo.zg_num) {uni.showToast({ title: "投票未结束！", icon: "none" });return;}}if (this.inve == "投票") {if (this.thistp) {uni.showToast({ title: "您已投过票了！", icon: "none" });return;}var that = this;var url = "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=saveVote2_YD&userId=").concat((0, _service.getUserInfo)().userId, "&status=").concat(that.status, "&comment=").concat(that.comment, "&taskInstanceId=").concat(that.tiid, "&acceptId=").concat(that.acceptid, "&docId=").concat(that.doc_id, "&tokenName=").concat(that.inve, "&pdid=").concat(that.pdid, "&nodeCode=").concat(that.nodecode);} // 				if(this.inve == "分网点审批（一般新增）"){
      // 				}
      if (this.inve == "县审查") {if (this.tprList.length < 2 || this.tprList.length > 3) {uni.showToast({ title: "请选择两个或三个投票人", icon: "none" });return;}this.tprList = (0, _util.deteleObject)(this.tprList);var a = [];this.tprList.map(function (item) {a.push(item.code);});this.inveder = a.join(',');var that = this;var url = "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=approveYX_YD&userId=").concat((0, _service.getUserInfo)().userId, "&taskInstanceId=").concat(that.tiid, "&mainId=").concat(that.mianId, "&pdId=").concat(that.pdid, "&piId=").concat(that.piid, "&docId=").concat(that.doc_id, "&comment=").concat(that.comment, "&status=").concat(that.status, "&userIds=").concat(that.inveder, "&acceptId=").concat(that.acceptid);}if (this.inve == "县审议") {if (!this.isvoti) {uni.showToast({ title: "请先投票！", icon: "none" });return;}if (this.dataInfo.yj_num != this.dataInfo.zg_num) {uni.showToast({ title: "投票未结束！", icon: "none" });return;}} // 				if(this.inve == "县审批"){
      // 				}
      // 				if(this.inve == "分网点批复确认"){
      // 				}
      uni.showLoading({ title: "提交中", mask: true });console.log(url, " at pages\\application\\investigation5\\investigation5.vue:986");uni.request({ url: url, success: function success(res) {var data = res.data;if (data.code == 1) {uni.hideLoading();uni.showToast({ title: data.message, duration: 3000, mask: true });setTimeout(function () {uni.reLaunch({ url: "/pages/tabBar/information/information" });}, 3000);} else if (data.code == 2) {uni.showToast({ title: data.message, icon: "none", duration: 3000, mask: true });}}, fail: function fail(err) {console.log(err, " at pages\\application\\investigation5\\investigation5.vue:1003");} });} } };exports.default = _default;
