...
var uniPopup = function uniPopup() {return __webpack_require__.e(/*! import() | components/uni-popup/uni-popup */ "components/uni-popup/uni-popup").then(__webpack_require__.bind(null, /*! @/components/uni-popup/uni-popup.vue */ "E:\\project\\myApplication\\components\\uni-popup\\uni-popup.vue"));};var tkiFileManager = function tkiFileManager() {return __webpack_require__.e(/*! import() | components/tki-file-manager */ "components/tki-file-manager").then(__webpack_require__.bind(null, /*! @/components/tki-file-manager.vue */ "E:\\project\\myApplication\\components\\tki-file-manager.vue"));};var _default = { components: { tkiFileManager: tkiFileManager, uniPopup: uniPopup }, data: function data() {return { index: 0, array: [], //['新增', '续贷', '收回再贷']
      ...
      userIds: "", seq: "", isUp: false, ishold: false };}, onNavigationBarButtonTap: function onNavigationBarButtonTap(e) {this.isKeep();}, onLoad: function onLoad() {this.businessData.applay_date = (0, _util.formateDate)(new Date(), "Y-M-D"); // 			this.businessData.bandsman = "单人"
    ...
    var that = this;uni.request({ url: "http://" + (0, _config.baseIp)() + "/ams/system/distribute.htm?module=pullDown&XLX=sbType", success: function success(res) {var data = res.data;that.arrayInfo = data.object; // console.log(data)
        ...
        data = data.object.map(function (item) {return item.name;});var data2 = [].concat(data);data.unshift("请选择拟初评结果");data2.unshift("请选择上季度拟初评结果");that.array5 = data;that.array6 = data2;} });uni.request({ url: "http://" + (0, _config.baseIp)() + "/ams/system/distribute.htm?module=check_dept", success: function success(res) {var data = res.data;that.array3Info = that.array7Info = data.object;var dataInfo = data.object.map(function (item) {return item.dept_name;});var data2Info = [].concat(dataInfo);dataInfo.unshift("请选择营销责任人");that.array3 = dataInfo;data2Info.unshift("请选择协助调查岗");that.array7 = data2Info;} });this.getMonryList();}, beforeDestroy: function beforeDestroy() {return false;}, methods: { // 附件上传
    doUpload: function doUpload() {if (!this.ishold) {uni.showModal({ title: '您还没有保存', content: '您还没有保存数据，请先保存！', success: function success(res) {if (res.confirm) {// console.log('用户点击确定');
            } else if (res.cancel) {// console.log('用户点击取消');
            }} });return;}if (this.path.length == 0) {uni.showToast({ title: "请先选择资料！", icon: "none" });return;}uni.showLoading({ title: "上传中", mask: true });var request = [];this.path.map(function (item, index) {request[index] = { name: index + 1, uri: "" };});this.path.map(function (item, index) {request[index].uri = item;});var that = this;var url = "http://".concat((0, _config.baseIp)(), "/ams/system/distribute.htm?module=PJupload&userId=").concat((0, _service.getUserInfo)().userId, "&mainId=").concat(that.seq); // return;
      uni.uploadFile({ url: url, filePath: "", name: '', files: request, success: function success(res) {if (typeof res.data == 'string') {var data = JSON.parse(res.data);} else {var data = res.data;}if (data.code == 1) {that.isUp = true;
            uni.hideLoading();
    ...
    submitAcce: function submitAcce() {
      if (!this.ishold) {
        uni.showModal({
          ...
          success: function success(res) {
            if (res.confirm) {
              console.log('用户点击确定', " at pages\\application\\creditGranting\\creditGranting.vue:632");
            } else if (res.cancel) {
              console.log('用户点击取消', " at pages\\application\\creditGranting\\creditGranting.vue:634");
            }
          } });
        ...
        uni.request({
          ...
          fail: function fail(err) {
            console.log(err, " at pages\\application\\creditGranting\\creditGranting.vue:672");
          } });
    ...
    resultPath: function resultPath(e) {this.path.push("file://".concat(e));},
    ...
    retrieval: function retrieval() {
      ...
      if (this.businessData.customer_name == "") {
        ...
        uni.request({
          ...
          fail: function fail(err) {
            console.log(err, " at pages\\application\\creditGranting\\creditGranting.vue:725");
            uni.hideLoading();
          } });
        ...
        success: function success(res) {
          if (res.confirm) {
            ...
            uni.request({
              ...
              fail: function fail(err) {
                console.log(err, " at pages\\application\\creditGranting\\creditGranting.vue:864");
                uni.hideLoading();
              } });
