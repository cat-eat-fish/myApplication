<template>
	<view class="ratingProcess">
		...
		<view class="ratingProcess-con">
			...
			<!-- <view class="item" >
				<view class="text-l">外出时长</view>
				<view class="text-r">{{dataInfo.outduration}}</view>
			</view> -->
			<view class="item" >
				<view class="text-l">外出类型</view>
				<view class="text-r">{{dataInfo.out_type}}</view>
			</view>
			<view class="item" >
				<view class="text-l">外出开始时间</view>
				<view class="text-r">{{dataInfo.out_start}} </view>
			</view>
			<view class="item" v-if="dataInfo.out_end">
				<view class="text-l">外出结束时间</view>
				<view class="text-r">{{dataInfo.out_end}} </view>
			</view>
			<view class="item" >
				<view class="text-l">外出事由</view>
				<view class="text-r">{{dataInfo.out_reason}}</view>
			</view>
			<view class="item" >
				<view class="text-l">附件</view>
				<view class="text-r">
					...
					success(res){
						...
						uni.request({
							...
							success(res2){
								...
								data2.out_type = data2.outtype == '1' ? '公事' : data2.outtype == '2' ? "私事" : "";	
								var i = data2.imgpath.indexOf("ams");
								var img = data2.imgpath.substring(i)
								
								img = img.replace(/\\/g, '/')
								var img2 = img.replace(/\\/g, '/')
								data2.picture = `${baseIp()}/${img2}`;
								that.dataInfo = data2;
								data.map((item)=>{
								...
								data.unshift({title:`${data2.name} -- ${data2.out_reason}`,desc:`我发起的`});
								that.list2 = data;
							}
						})
					}
