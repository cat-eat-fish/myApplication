...
<script>
	...
	export default {
		...
		methods:{
			...
			submitAcce(){
				var that = this;
				var url =`${baseIp()}/ams/system/distribute.htm?module=approveDoc_YD&acceptId=${this.acceptid}&taskInstanceId=${this.tiid}&pdId=${this.pdid}&docId=${this.doc_id}&piId=${this.piid}&userId=${getUserInfo().userName}&userId=${getUserInfo().userName}&comment=${this.comment}&status=${this.status}&hr=true`;
