<template>
	<view class="financialTrial">
		...
		<view class="btns">
			<button type="primary" @click="isKeep">保存</button>
	...
	export default {
		...
		methods:{
			...
			isKeep(){
				...
				var url = `${baseIp()}/ams/system/distribute.htm?module=addLeave_YD&id=${getUserInfo().userName}&type=${this.type}&branchId=${this.dept_id}&busType=05&name=${uploadData.name}&position=${uploadData.station}&mobie=${uploadData.phone}&day=${uploadData.num}&leaveType=${uploadData.title}&applayDate1=${uploadData.start}&applayDate2=${uploadData.end}&leaveReason=${uploadData.cause}&hr=true`;
				console.log(url)
				var that = this;
				uni.showModal({
					title: '是否保存',
			...
			submitAcce(){
				if(!this.ishold){
					...
					var that = this,
					url = `${baseIp()}/ams/system/distribute.htm?module=submitLeaveMainInfo_YD&Id=${that.seq}&userId=${getUserInfo().userName}&hr=true`;
					uni.request({
						url,
						success(res){
							var data = res.data;
							...
							if(data.code == 1){
								...
								// var url2 = `http://${baseIp()}/ams/system/distribute.htm?module=submitLeaveMainInfo_YD&clientId=${getCid()}&userId=${getUserInfo().userName}`
								// console.log(url2)
								// uni.request({
								// 	url:url2,
								// 	success(res2){
